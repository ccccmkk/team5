...
- **먼저 현재 브랜치와 최근 변경을 확인한다.** 최소한 `git status`와 `git log -5 --oneline`을 보고 시작한다.
  다른 사람/에이전트가 바로 전에 바꾼 내용을 오래된 대화나 PR 본문보다 우선한다.
- **현재 작업과 관련된 파일을 직접 읽고 수정한다.** 오래된 PR 본문을 매 작업마다 전부 다시 읽는 것을 기본 절차로 삼지 않는다.
  PR의 결정 근거가 계속 필요한 내용이면 PR에만 두지 말고 스펙, 브랜드 가이드, 이 파일처럼 실제 작업 때 읽는 문서로 옮긴다.
- 최근 커밋에서 관련 파일이 바뀌었다면 그 diff/파일을 먼저 확인한다. 특히 스펙, DB 마이그레이션,
  `data/models.ts`, 디자인 규칙 변경은 이전 기억보다 현재 `main`의 내용을 기준으로 판단한다.
