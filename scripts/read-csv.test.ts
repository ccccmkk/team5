...
describe("parseSeedCsv", () => {
  it("한 줄을 후기 객체로 바꾼다", () => {
    const csv = `${HEADER}\n느긋한수달,178,74,32,58,97,80,501,32,0,-1,0,1,4,허벅지가 낀다`;
  ...
  it("빈 선택 항목은 스냅샷에서 아예 빠진다", () => {
    const csv = `${HEADER}\n조용한오리,168,58,29,,,,501,30,1,0,0,2,3,무난하다`;
    const [review] = parseSeedCsv(csv);
  ...
  it("한줄평에 쉼표가 있어도 잘리지 않는다", () => {
    const csv = `${HEADER}\n조용한오리,168,58,29,,,,501,30,0,0,0,0,4,허리는 맞고, 허벅지도 맞는다`;
    const [review] = parseSeedCsv(csv);
  ...
  it("실제 시드 CSV 파일이 파싱되고 DB 제약 범위 안이다", async () => {
    ...
    for (const r of reviews) {
      expect(["501", "517"]).toContain(r.modelId);
