...
/**
 * 대조한 날짜. 비어 있으면 "아직 검증 안 됨"을 뜻한다.
 *
 * 2026-08-13 현재 여전히 비어 있다. levi.com은 자동 요청을 403으로 막고,
 * 공개된 2차 자료로 확인할 수 있었던 것은 BASE_SIZES의 허리·엉덩이뿐이다
 * (W32 ≈ 81cm 허리, W34 ≈ 86~89cm 허리 / 102~104cm 엉덩이 — 아래 값과 일치).
 * 모델별 허벅지·엉덩이 보정치는 대조하지 못했다. 리바이스가 핏별로 공개하는
 * 것은 밑위·무릎·밑단이지 허벅지 수용치가 아니라, 같은 축의 공식 수치가 없다.
 *
 * 그래서 아래 보정치는 "팀이 정리한 핏 특성을 순서대로 옮긴 추정"이다.
 * 릴렉스드가 슬림보다 넉넉하다는 대소 관계는 맞지만 절대값은 근거가 없다.
 *
 * 다만 이 표는 화면에 나오지 않는다. 추천(recommendSize)은 후기 데이터만
 * 쓰고, 이 표는 시드 후기의 핏 값을 유도할 때만 쓰인다. 즉 지금 영향받는 것은
 * 합성 데이터의 현실성이지 사용자에게 보이는 추천이 아니다.
 */
const CHECKED_AT = "";
