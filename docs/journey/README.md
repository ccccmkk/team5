...
## 6차 — 사용자가 병목을 정확히 짚어줬다

외부에 뿌리자 바로 피드백이 왔다.

> *"자신의 정확한 허벅지 둘레나 밑위길이를 모르겠어서 입력을 못하겠다거나,
> 수치를 직접 입력하기보단 슬림/표준/하체발달 등 체형 옵션처럼 선택형 옵션을
> 제공해주면 좋겠다는 의견이 있었습니다."*

**같은 것을 GA도 가리키고 있었다.** `profile_start` 10건인데 완료가 그보다 적었다.
어디서 막히는지 숫자로는 "체형 입력 어딘가"까지만 알 수 있었는데, 사용자가
**정확히 어느 칸에서 왜** 막히는지 말해줬다.

> **배운 것**: 숫자는 어디가 새는지 알려주고, 사람은 왜 새는지 알려준다.
> 둘 중 하나만 있었으면 엉뚱한 곳을 고쳤을 것이다. 에어비앤비 사례에서
> "돈을 내고 쓴 고객의 이야기를 많이 들어봐야 한다"고 한 게 이 얘기였다.

### 고친 방법

세 항목 모두 옵션을 먼저 두고, 아는 사람만 직접 입력한다.
...
고르면 성별·몸무게·키에서 치수를 계산해 채운다. 74kg 남성이 "하체발달"을 고르면
57cm가 들어간다. 계수는 시드 생성기와 같은 값을 쓴다 — 두 곳이 어긋나면 합성
데이터와 실제 입력이 다른 분포를 갖는다.

### 정확도를 부풀리지 않기로 했다

고른 값은 직접 잰 값보다 거칠다. 같은 옵션을 고른 사람끼리는 같은 값이 나와
유사도가 실제보다 가깝게 계산된다.

유사도 계산 자체는 손대지 않았다(양쪽 스냅샷을 모두 고쳐야 해서 범위가 커진다).
대신 화면의 "입력 정확도"에서 **절반만 인정**하고 그 이유를 화면에 적었다.
직접 입력이면 85%, 골라서 채우면 73%다.

여기서 눈속임을 했다면 사용자는 자기 입력이 실제보다 정밀하다고 믿게 된다.
숫자를 좋아 보이게 만드는 것과 정직하게 만드는 것 사이에서 후자를 골랐다.

### 배포 순서 함정을 미리 막았다

배포는 main에 머지되는 즉시 도는데, DB 마이그레이션은 사람이 대시보드에서 돌린다.
그 사이에 들어온 사람은 체형 저장이 **통째로 실패**한다. 인스타 유입이 들어오는
중이라 실제로 겪을 수 있는 일이었다.

그래서 컬럼이 없으면 그 항목만 빼고 다시 시도하도록 했다. 읽기도 마찬가지다 —
조회가 실패하면 기존 사용자에게 "체형을 입력한 적 없음"으로 보여 처음부터 다시
입력하게 된다.

**여기서도 한 번 틀렸다.** 오류 코드를 `42703`만 보고 있었는데 되돌림 경로를
타지 않았다. 직접 요청을 보내 보니 쓰기는 PostgREST가 스키마 캐시에서 먼저 걸러
`PGRST204`를 준다. 둘 다 넣고, 컬럼이 없는 상태로 E2E를 돌려 전체 흐름이 도는
것을 확인했다.

### 안 한 것

사진 첨부 의견도 있었지만 넣지 않았다. 제안한 쪽에서도 "꼭 반영은 안 되어도
괜찮다"고 했고, Storage·용량·부적절 이미지 처리가 따라와 지금 넣기엔 크다.
당장 급한 입력 이탈은 옵션으로 줄어든다.
