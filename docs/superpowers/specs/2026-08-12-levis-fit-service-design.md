...
- **구글 계정 연결 (선택)** — 후기를 쓸 때는 요구하지 않는다. 다 쓴 뒤 `/me`에서 제안한다 (→ §3.1). 목업 버튼은 두지 않는다 — 누를 수 없는 UI는 정보가 아니라 잡음이다
...
대한 소유권을 잃는다. `/me`의 **구글 계정 연결**이 이것을 푼다.
...
후기가 0건인 초기에는 그 손실이 소유권 문제보다 크다. 그래서 쓰는 것은 익명으로 두고,
다 쓴 다음 이득이 분명해진 시점에 제안한다.
